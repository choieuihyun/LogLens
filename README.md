...
- **계층 트리** — 조직도·주소록처럼 "누르면 그 아래를 불러오는" 화면은 로그가 평평하게
  쌓입니다. 각 줄에 자기 부모가 적혀 있으면 계층을 되살릴 수 있습니다.
  **해당 도메인 탭에서 로그와 나란히** 보입니다 (별도 화면도, 전환도 아닙니다).
  줄을 누르면 파일 탐색기처럼 그 자리에서 펼쳐지고 접힙니다.
  - 뷰어는 **언제나 부분 트리**만 봅니다 — 사용자가 펼친 것만 로그가 남으니까요.
    그래서 `+4 미관측` 처럼 **아직 못 본 가지가 몇 개인지** 표시합니다.
    앱이 세는 기준이 다르면 실제보다 크게 나올 수 있어서, 단정하지 않고 툴팁에 근거를 적습니다
  - `depth` 는 트리를 세우는 데 쓰지 않고 **검증**에만 씁니다. `parent` 로 계산한 깊이와
    어긋나면 `parent` 필드가 잘못 들어오고 있다는 신호라 화면에 경고로 띄웁니다
...
| | |
|---|---|
| 뷰어 (파서·집계·설정·자동생성·계층) | 103개 테스트 통과 |
